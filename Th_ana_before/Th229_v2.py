...
def ofs_def(sig=3,ID=False,sID=80000,eID=269306):
    i = 0
    nfl = get_fl(s="n")
    for fn in nfl:
        print(fn)
    ...
    if ID == True:
        ofs = ofs[sID:eID]
        ofs_std = ofs_std[sID:eID]
    ofs = np.median(n,axis=1)
    ofs_std = np.std(n,axis=1)
    ofs_all_avg = np.median(ofs)
    ofs_all_std = np.std(ofs)
    omask = (ofs_all_avg-sig*ofs_all_std<ofs) & (ofs_all_avg+sig*ofs_all_std>ofs)
    ttrig = 136600
    imask =   (np.array(range(len(ofs))) < ttrig)
    mask = (omask) & (imask)
    return ofs,ofs_std,ofs_all_avg,ofs_all_std,mask
    
                    
    
def ph_def(sig=3,ID=False,sID=80000,eID=269306):
    i = 0
    pfl = get_fl(s="p")
    for fn in pfl:
        i = i + 1
...
def make_hd(fn=h5_fn,sig=3,ID=False,sID=80000,eID=269306):
    with h5py.File(fn) as f:
        p_mask = f["double_mask"][:]
        if ID == True:
            p_mask = f["double_mask"][sID:eID]
        ofs,ofs_std,ofs_all_avg,ofs_all_std,o_mask = ofs_def(sig=sig,ID=ID,sID=sID,eID=eID)
        print(o_mask)
        print(p_mask)
        ph = ph_def(sig=sig,ID=ID,sID=sID,eID=eID)
...
def tmpl_psel(fn=h5_fn,ID=False,sID=80000,eID=269306):
    fl = get_fl(s="p")
    with h5py.File(fn) as f:
        ...
        for fnl in fl:
            t,p=Util.fopen(fnl)
            if ID == True:
                if "ph_mask" in locals():
                    if sID < (i + len(p)) :
                        ph_mask = f["mask/Am241"][sID:i+len(p)]
                        ofs_mask = f["mask/offset"][sID:i+len(p)]  
                else :              
                    ph_mask = f["mask/Am241"][i:i+len(p)]
                    ofs_mask = f["mask/offset"][i:i+len(p)]                
            else:
                if i == 0:
    ...
    with h5py.File(fn) as f:
        ...
        for fnl in fl:
            ...
            if i == 0:
                ...
                ph_mask = f["mask/Am241"][i:i+len(n)]
                ofs_mask = f["mask/offset"][i:i+len(n)]
            i = i + len(n)
            mask = (ph_mask) & (ofs_mask)
...
def tmpl_sn(ms="Am241",fn=h5_fn,ge=False):
    with h5py.File(fn) as f:
...
def ofs_hist(fn=h5_fn,bs=10000):
    with h5py.File(fn) as f:
        ofs = f["data/offset"]
        fig = plt.figure(figsize=(8.0,6.0))
        ax = fig.add_subplot(111)
        ax.hist(ofs,bins=bs,histtype="step")
...
def pha_ofs_lin(fn=h5_fn,bs=1000,secor=False):
    with h5py.File(fn) as f:
        ...
        if secor == True:
            ...
            ax1.hist(pha[ofs_mask],orientation="horizontal", bins=bs, histtype="step")
        ax1.invert_xaxis()
        ax1.set_ylabel("Pulse height amplitude",fontsize=16)
        ax2 = plt.subplot(gs[1],sharey=ax1)
        if secor == True :
            ax2.plot(ofs[ofs_mask], pha, ".")
        else :
...
def pha_ofs_lin2(fn=h5_fn,bs=1000,pha1=None,pha2=None,ofs=None):
    with h5py.File(fn) as f:
        ofs_mask = f["mask/offset"][:]
        fig = plt.figure(figsize = (16,8))
        gs = GridSpec(1,2,width_ratios=(1,4))
        ax1 = plt.subplot(gs[0])
        ax2 = plt.subplot(gs[1],sharey=ax1)
        ax1.hist(pha1,orientation="horizontal", bins=bs, histtype="step")
        ax1.hist(pha2,orientation="horizontal", bins=bs, histtype="step")
        ax1.invert_xaxis()
        ax1.set_ylabel("Pulse height amplitude",fontsize=16)
        ax2 = plt.subplot(gs[1],sharey=ax1)
        ax2.plot(ofs[ofs_mask], pha1, ".")
        ax2.plot(ofs[ofs_mask], pha2, ".")
        ax2.set_title("Offset vs PHA",fontsize=16)
        ax2.set_xlabel("Offset[V]",fontsize=16)
        plt.setp(ax2.get_yticklabels(), visible=False)
        plt.subplots_adjust(wspace=0.1)
...
def pha_hist(fn=h5_fn,bs=500,ran=None,vv=None,x="ene",ID=True,se=False):
    with h5py.File(fn) as f:
        ...
        if x == "pha":
            ...
            ax.hist(pha*30.973/2.20228,range=(ran),bins=bs)
...
def cal_se(fn=h5_fn,pa=[1.5,1.7],all=True,sr=[-0.4,-0.1]):
    with h5py.File(fn) as f:
        ofs_mask = f["mask/offset"]
...
def fits2xspec(binsize=1, exptime=1, fwhm=0.0001, gresp=False, garf=False, name='test', arfname='test.arf', TEStype='TMU524', Datatype='PHA', chan='ch65',gen=True,pha=None):
    ...
    with h5py.File(h5_fn) as f:
        if gen == True:
            ...
            #mask = f["mask/offset"][:]
            #pha = f["analysis/pha"][:][mask]*26.3*1e+3/1.45
            pha = f["analysis/se_cor/pha"][:]*26.3*1e+3/1.45
        # separate bins
        if Datatype=='PHA':
...
def tmpl_apsel(fn=h5_fn,ID=False,sID=0,eID=82500):
    fl = get_fl(s="p")
    with h5py.File(fn) as f:
        if "analysis" in f.keys():
            del f["analysis"]
        ...
        print(p.shape)
        print(len(p[mask]))
        f.create_dataset("analysis/tmpl/pulse/sel_pulse",data=p[mask])
        
def tmpl_ansel(fn=h5_fn,ID=False,sID=0,eID=82500):
    fl = get_fl(s="n")
    with h5py.File(fn) as f:
        if "noise" in f["analysis/tmpl"].keys():
            del f["analysis/tmpl/noise"]
        ...
        print(n.shape)
        print(len(n[mask]))
        f.create_dataset("analysis/tmpl/noise/sel_noise",data=n[mask])
        if "time" in f["data"].keys():
            del f["data/time"]
        f.create_dataset("data/time",data=t)

def pha_aopt(fn=h5_fn,ID=False,sID=0,eID=82500):
    fl = get_fl(s="p")
    with h5py.File(fn) as f:
        if "pha" in f["analysis"].keys():
            del f["analysis/pha"]
        ...
        del f["analysis/pha"]
        if ID == True:
            print(len(pha[sID:eID]))
            f.create_dataset("analysis/pha",data=pha[sID:eID])
        else:
            f.create_dataset("analysis/pha",data=pha)
        ...
        del f["analysis/ps"]
        if ID == True:
            f.create_dataset("analysis/ps",data=ps[sID:eID])           
        else:
            f.create_dataset("analysis/ps",data=ps)
