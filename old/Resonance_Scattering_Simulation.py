...
class GeneralFunction:
    ...
    def gaussian_fitting_with_plot(self, data, ax=None, title=None):
        n, bins = self.histogram(data, binsize=None)
...
class PKS:
    ...
    def load_pks_setting(self, method='sanders_george_digit'):
        if method == 'beta_model_chen':
            ...
            r = np.linspace(0, 2500, 400)
            r_ne, ne = self.load_setting('sanders_george_ne_digit.txt')
            r_T, T = self.load_setting('sanders_george_Te_digit.txt')
            r_Z = r_ne
            Z = np.full(len(r_Z), 0.4)
            Z *= 4.68/3.27     # for abundance conversion from Anders & Grevesse (1989) to Lodders et al. (2009)
    ...
    def _compute_resonance_scattering(self, line_state, rmax, Emin, Emax, dn, dL, f_ne, f_T, f_Z, velocity, style="tau", broadening=False, delE=None):
        ...
        def compute_prob(R, E):
            if self.load_furukawa:
                tau_factor = (f_ne(R) *u.cm**(-3) * self.Fe_H * f_Z(R) * self.atom.iz(R) * dL).to('cm**-2')
            else:
                tau_factor = (f_ne(R) *u.cm**(-3) * self.Fe_H * f_Z(R) * self.atom.iz(f_T(R)*u.keV) * dL).to('cm**-2')
            if broadening:
...
class RadiationField:
    ...
    def apec_field(self,kT=5, turbulent_velocity=0, abundance=0.4):
        ...
        sess.set_abund(el_list, abundance)
    ...
    def generate_apec_photon(self, kT=4, Emin=6.60, Emax=6.75, dE=1e-3, velocity=0, abundance=0.4, size=1000, filename='photon.hdf5', cluster_name='None', method='None', radius=None, save=True):
        size = int(size)
        ebins, spec = self.apec_field_abund_Fe(kT=kT, turbulent_velocity=velocity, abundance=abundance)
    ...
    def make_fits(self):
        ...
        photon = self.generate_apec_spectrum_from_hdf5(size=int(1e6),sampling_Emin=6.5,sampling_Emax=8.0,radius=0,turbulent_velocity=100,radial_list=[0],seed_filename='perseus_apec_spectrum')
        print(photon)
        plt.hist(photon, bins=1000,histtype="step")
        plt.show()
        D.fits2xspec(binsize=0.5, exptime=1, fwhm=0.0001, name=f'spectrum_abundance_test', TEStype='TMU542', Datatype='PHA', pha=photon*1e3)
    ...
    def _plot_apec_field(self,kT=5.5):
        ...
        ebins, spec = self.apec_field_abund_Fe(kT, 0, 0.6)
        norm = self.integrate_spectrum(ebins, spec)
        print(norm)
...
class Simulation:
    ...
    def __init__(self, physics_list=['RS', 'TS'], rmin=0.1, rmax=1000, division=100, Emin = 6500, Emax = 8000, 
                 ...
                 ne_dummy=0.01, kT_dummy=5.0, Z_dummy=0.4, comparison=False
                 ):
        '''
...
#? for thesis tau plot
    ...
    def tau_v_dependence(self):
        ...
        for i, line_list in enumerate(tau_result.keys()):
            plt.plot(turb_list, tau_result[line_list], label=f'{line_list}', color=plt.cm.viridis(i/len(tau_result.keys()))) 
    ...
    def integrate_rs_tau(self, f_ne, f_T, f_Z, v):
        self.initialize_for_tau()
        energy_values = np.linspace(6500,8000, 100)
        R_min, R_max = 0, 100
        physics = Physics()
        for line_list in ['w']:
    ...
    def initialize(self):
        ...
        self.physics        = Physics(line_state='w',atomdb_version=self.atomdb_version, abundance_table=self.abundance_table,load_furukawa=True)
        # set the line manager, this class manage the resonance scattering lines
        print('Set the line manager', flush=True)
        self.line_manager   = RSLineManager(rs_line_list=self.rs_line_list, dE=self.rs_dE, atomdb_version=self.atomdb_version, abundance_table=self.abundance_table,load_furukawa=True)
    ...
    def initialize_for_tau_E(self):
        ...
        # set the line manager, this class manage the resonance scattering lines
        self.line_manager   = RSLineManager(rs_line_list=self.rs_line_list, dE=self.rs_dE)
        # set the ion field, this class manage the ion velocity
        self.ion            = IonField()
        # definition of the probability or mean-free path function of the resonance scattering and thomson scattering.
...
class SimulationResult:
    ...
    def load_result(self, keyname=None):
        ...
        # print(self.RS_position)
        # position = np.array(self.position)
        # r = position[:,0]
        # theta = position[:,1]  # 極角（z軸からの角度）: [0, π]
        # phi = position[:,2]    # 方位角（x軸からxy平面内の角度）: [0, 2π]
        ...
        # ax.scatter(x, y, z, s=0.1, alpha=0.2)

        # ax.set_xlabel("X")
        # ax.set_ylabel("Y")
        # ax.set_zlabel("Z")
        # ax.set_title("Photon Positions (3D Polar Coordinates)")

        # plt.tight_layout()
        # plt.show()
        # plt.hist(position[:,0], bins=100, histtype='step', color='black', label='all photons')
        # plt.show()
        # plt.hist(position[:,1], bins=100, histtype='step', color='blue', label='unscattered photons')
        # plt.show()
        # plt.hist(position[:,2], bins=100, histtype='step', color='red', label='scattered photons')
        # plt.show()

# class VectorCalculator:
#     def __init__(self, r1, theta1, phi1, r2, theta2, phi2, sphere_radius):
#         self.point1 = self.spherical_to_cartesian(r1, theta1, phi1)
#         self.point2 = self.spherical_to_cartesian(r2, theta2, phi2)
...
#         vector = self.point2 - self.point1  # shape: (3, n)
#         norms = np.linalg.norm(vector, axis=0)
#         return vector / norms  # shape: (3, n)

#     def find_intersection_point(self):
#         """Find the intersection point of the vector with the sphere (ベクトル対応)."""
#         if np.allclose(self.direction_vector, 0):
#             raise ValueError("Direction vector is a zero vector, cannot find intersection.")
#         a = np.einsum('ij,ij->j', self.direction_vector, self.direction_vector)
#         b = 2 * np.einsum('ij,ij->j', self.point1, self.direction_vector)
#         c = np.einsum('ij,ij->j', self.point1, self.point1) - self.sphere_radius**2
#         discriminant = b**2 - 4 * a * c
#         if np.any(discriminant < 0):
#             raise ValueError("No intersection with the sphere for some rays.")
#         t = (-b + np.sqrt(discriminant)) / (2 * a)
#         intersection_point = self.point1 + t * self.direction_vector  # shape: (3, n)
#         if np.any(np.all(np.isclose(intersection_point, 0), axis=0)):
#             raise ValueError("Some intersection points are at the origin, cannot normalize.")

#         return intersection_point

#     def calculate_angle_with_tangent_plane(self):
#         """Calculate the angle between the vector and the tangent plane at the intersection (ベクトル対応)."""
#         intersection_point = self.find_intersection_point()  # shape: (3, n)
#         normal_vector = intersection_point / np.linalg.norm(intersection_point, axis=0)  # shape:
#         direction_unit_vector = self.direction_vector / np.linalg.norm(self.direction_vector, axis=0)
#         if np.all(np.all(np.isclose(direction_unit_vector, normal_vector), axis=0)):
#             print("Warning: Some direction vectors and normal vectors are parallel.")
#         cos_theta = np.einsum('ij,ij->j', self.direction_vector, normal_vector)
#         cos_theta /= (np.linalg.norm(self.direction_vector, axis=0) * np.linalg.norm(normal_vector, axis=0))
#         cos_theta = np.clip(cos_theta, -1.0, 1.0)
#         angle_rad = np.arccos(cos_theta)
#         angle_with_tangent_plane = angle_rad
#         return np.degrees(angle_with_tangent_plane)
...
class Detector:
    ...
    '''
    def __init__(self, cluster='perseus', omega_s=0.73, H0=70, sphere_radius=825, savefile='simulation_perseus_digit.hdf5'):
    ...
    def calculate_photon_degree(self,keyname='None',sphere_radius=825):
        """
        ...
        # === RSがない場合の最終位置を計算 ===
        no_RS_position = photon.propagate_to_rmax_array(initial_position, initial_direction, sphere_radius)
    ...
    def make_spectrum_divided_angle(self, keyname='None', sphere_radius=825, angle_spec=False):
        filename = self.savefile
        ...
        if keyname == 'None':
            ...
            keyname = keyname
        print(hdf5_file[keyname].attrs["velocity"])
        self.deg_hist, self.energies, self.velocity, self.scatter, self.deg_hist_noRS, initial_energy, initial_position, RS_position, position = self.calculate_photon_degree(keyname, sphere_radius)
        ...
        self._plot_angle()
        self.arcmin_hist = self.deg2arcmin2(self.deg_hist)
        self.arcmin_hist_noRS = self.deg2arcmin2(self.deg_hist_noRS)
        if angle_spec:
    ...
    def _plot_angle(self):
        ...
        plt.show()
        arcmin_hist = self.deg2arcmin2(self.deg_hist)
        arcmin_hist_noRS = self.deg2arcmin2(self.deg_hist_noRS)
...
#? : Not  
    ...
    def deg_hist_multi(self, filename='simulation_perseus_digit.hdf5', sphere_radius=825):
        keynames = h5py.File(filename, 'r').keys()
        for keyname in keynames:
            self.plot_deg_hist(filename=filename, keyname=keyname, sphere_radius=sphere_radius)
...
def tau_plot():
    s = Simulation(physics_list=['RS'], rmin=0, rmax=820, division=400, Emin = 6500, Emax = 6800, dn = 1000,  size=int(1e4), rs_dE=20, turbulent_velocity=0, rs_line_list=['w'], cluster_name='perseus', step_dist=1, test_mode=False, tau_mode="effective")
    #s.integrate_rs()
    #plt.show()
    s.tau_v_dependence()
