...
class IVAnalysis:
    ...
    def __init__(self,Min,Mfb):
        ...
        self.Min = Min
        self.Mfb = Mfb
        # self.Min = 105.31e-12  #[H]
        # self.Mfb = 85.79e-12  #[H]
    ...
    def Rn_define(self,Ibias=800,per=0.25):
        for e,i in enumerate(self.data["data"].keys()):
            if e == 0:
                self.Rn = self.data["data"][i]["Rtes"][self.getNearValueID(self.data["data"][i]["Ibias"]*1e+6,Ibias)]
            else:
...
## Append for COMSOL simlation(2021.12.21) ##
    ...
    def all_analysis(self,Iunit,savehdf5,Ibias,mes_number,Rpar): 
